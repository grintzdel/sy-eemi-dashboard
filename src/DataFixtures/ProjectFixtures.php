...
final class ProjectFixtures extends Fixture
{
    ...
    {
        $projectTemplates = [
            ['Refonte du site web', 'Modernisation complète du site institutionnel'],
            ['Application mobile', 'Développement d\'une application mobile native'],
            ['Migration cloud', 'Migration de l\'infrastructure vers AWS'],
            ['Tableau de bord analytics', 'Création d\'un dashboard de métriques'],
            ['API REST v2', 'Refonte de l\'API avec GraphQL'],
            ['Système de paiement', 'Intégration de Stripe et PayPal'],
            ['Module de facturation', 'Automatisation de la facturation client'],
            ['Plateforme e-learning', 'Création d\'une plateforme de formation en ligne'],
        ...
        for ($month = 1; $month <= 12; $month++)
        {
            ...
            for ($p = 0; $p < $projectsThisMonth; $p++)
            {
                ...
                $this->addReference('project_2024_' . $month . '_' . $p, $project);
            }
        }
        ...
        for ($month = 1; $month <= 11; $month++)
        {
            ...
            for ($p = 0; $p < $projectsThisMonth; $p++)
            {
                ...
                $manager->persist($project);
                $this->addReference('project_2025_' . $month . '_' . $p, $project);
            }
        }
    ...
    private function getRandomDateInMonth(int $year, int $month): \DateTimeImmutable
    {
        $day = rand(1, 28);
        $hour = rand(8, 18);
        $minute = rand(0, 59);

        return new \DateTimeImmutable("{$year}-{$month}-{$day} {$hour}:{$minute}:00");
    }
}
