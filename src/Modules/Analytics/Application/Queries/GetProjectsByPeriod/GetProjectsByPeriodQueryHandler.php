...
final readonly class GetProjectsByPeriodQueryHandler
{
    ...
    public function __invoke(GetProjectsByPeriodQuery $query): array
    {
        ...
        return $this->mapResults($result->fetchAllAssociative(), $period, $year);
    }
    ...
    private function mapResults(array $results, PeriodType $period, int $year): array
    {
        return array_map(
            fn(array $row) => new ProjectsByPeriodViewModel(
                period: $period->value,
                periodNumber: (int) $row['period_number'],
                periodLabel: $this->getPeriodLabel($period, (int) $row['period_number']),
                count: (int) $row['count'],
    ...
    private function getPeriodLabel(PeriodType $period, int $periodNumber): string
    {
        return match($period) {
